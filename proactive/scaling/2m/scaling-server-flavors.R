...
run_analysis = function(filename_utilization_cpu, 
	...
	file_name_capacity_cpu	<- paste("../cpu/data/capacity/", pred_type, "/server-", trace_number, "-capacity-planning-", ad_w_value_cpu, "-", pred.abr.labels[[policy]], "-TRUE-", adjustment_type, "-", slo_limit, "-", as.character(capacity_target_cpu), "-linear.dat", sep = "")
	capacity_trace_cpu	     <- read.table(file_name_capacity_cpu, header = T)
	
	file_name_capacity_mem	<- paste("../mem/data/capacity/", pred_type, "/server-", trace_number, "-capacity-planning-", ad_w_value_mem, "-", pred.abr.labels[[policy]], "-TRUE-", adjustment_type, "-", slo_limit, "-", as.character(capacity_target_mem), "-linear.dat", sep = "")
	capacity_trace_mem		<- read.table(file_name_capacity_mem, header = T)
		    
	file_name_scaling  	     <- paste(outpur_dir, "scaling/", pred_type, "/server-", trace_number, "-scaling-", ad_w_value_cpu, "_", ad_w_value_mem, "-", pred.abr.labels[[policy]], "-", instance_grain, "-", time_decision, "-", error_adjustment, "-", adjustment_type, "-", slo_limit, "-", as.character(capacity_target_cpu), "-", as.character(capacity_target_mem), "-", instance_size_ecu, "-", instance_size_ghz, "-", instance_size_mem, "-", ecu_factor, ".dat", sep = "")
