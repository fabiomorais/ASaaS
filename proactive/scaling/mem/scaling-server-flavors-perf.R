...
run_analysis = function(filename_utilization, filename_allocation, trace_number, trace_type, pred_type, num_period, policy, input_dir, outpur_dir, instance_size_mem, time_decision, instance_grain, instance_base, pred_grain, cap_grain, error_adjustment, adjustment_type, ad_w, slo_limit, capacity_target) {
  ...
  #Perfect	
  file_name_scaling        <- paste(outpur_dir, "scaling/", pred_type, "/server-", trace_number, "-scaling-0-PERFECT-60-55-FALSE-", as.character(capacity_target), "-linear-", instance_size_mem, ".dat", sep = "" )
  if (!file.exists(file_name_scaling)) {
    ...
    original_scaling_trace   <- calculate_scaling(original_capacity_trace, time_decision, instance_size_mem, instance_grain, instance_base, trace_number, "PERFECT", ad_w)
