...
run_analysis = function(filename_utilization, filename_allocation, trace_number, trace_type, pred_type, num_period, policy, input_dir, outpur_dir, instance_size_cpu, time_decision, instance_grain, instance_base, pred_grain, cap_grain, error_adjustment, adjustment_type, ad_w, slo_limit, capacity_target) {
  ...
  file_name_scaling  	<- paste(outpur_dir, "scaling/", pred_type, "/server-", trace_number, "-scaling-", ad_w, "-", pred.abr.labels[[policy]], "-", instance_grain, "-", time_decision, "-", error_adjustment, "-", adjustment_type, "-", slo_limit, "-", as.character(capacity_target), "-linear-", instance_size_cpu, ".dat", sep = "")

  if (!file.exists(file_name_scaling)) {
    
    scaling_trace	    <- calculate_scaling(capacity_trace, time_decision, instance_size_cpu, instance_grain, instance_base, trace_number, pred.abr.labels[[policy]], ad_w) 			
