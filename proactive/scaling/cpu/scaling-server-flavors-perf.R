calculate_scaling = function(capacity_trace, time_decision, instance_size_cpu, instance_grain, instance_base, trace_number, policy_abr, adj_w) {
     
     timestamps		<- capacity_trace$TIMESTAMP
     
     inst_number		<- max(instance_base, ceiling(capacity_trace$CAP_EST_TG[1] / instance_size_cpu))	
     inst_alloc		<- c()
     ...
     for (i in 2:nrow(capacity_trace)) {
          ...
          cap <- ceiling(capacity_trace$CAP_EST_TG[i] / instance_size_cpu)
...
run_analysis = function(filename_utilization, filename_allocation, trace_number, trace_type, pred_type, num_period, policy, input_dir, outpur_dir, instance_size_cpu, time_decision, instance_grain, instance_base, pred_grain, cap_grain, error_adjustment, adjustment_type, ad_w, slo_limit, capacity_target) {
     ...
     dff_alloc	<- if (file.exists(filename_allocation)) {
          ...
          data.frame(CPU_ALLOC = rep(16, nrow(dff)))
     }
     
     print(ad_w)
     
     timestamps	        <- dff$TIMESTAMP
     trace_util	        <- dff$CPU_UTIL
     trace_alloc	        <- dff_alloc$CPU_ALLOC
     ...
     names(df_alloc)		  <- c("TIMESTAMP", "CPU_ALLOC", "EXEC_TIME", "POLICY")
     ...
     names(tmp_df_trace)	<- c("TIMESTAMP", "CPU_UTIL", "EXEC_TIME", "POLICY", "PERIOD")
     
     new_df_trace		<- data.frame(tmp_df_trace$TIMESTAMP, tmp_df_trace$CPU_UTIL * tmp_df_alloc$CPU_ALLOC / 100, tmp_df_trace$EXEC_TIME, tmp_df_trace$POLICY, tmp_df_trace$PERIOD)
     names(new_df_trace)	<- c("TIMESTAMP", "CPU_UTIL", "EXEC_TIME", "POLICY", "PERIOD")
     
     #Perfect	
     file_name_scaling        <- paste(outpur_dir, "scaling/", pred_type, "/server-", trace_number, "-scaling-0-PERFECT-60-55-FALSE-", as.character(capacity_target), "-linear-", instance_size_cpu, ".dat", sep = "")
     if (!file.exists(file_name_scaling)) {
          
          print(paste("Server: ", trace_number, "- Perfect scaling"))
          
          file_name_capacity       <- paste(outpur_dir, "capacity/", pred_type, "/server-", trace_number, "-capacity-planning-0-PERFECT-FALSE-", as.character(capacity_target), "-linear.dat", sep = "")
          original_capacity_trace  <- read.table(file_name_capacity, header = T)
          
          original_scaling_trace   <- calculate_scaling(original_capacity_trace, time_decision, instance_size_cpu, instance_grain, instance_base, trace_number, "PERFECT", ad_w)
          write.table(original_scaling_trace, file = file_name_scaling, row.names = FALSE)
     }
}
...
instance_size_cpu	<- as.numeric(args[9])

input_dir			<- paste("../../prediction/cpu/data/pred/", pred_type, "/", sep = "")
outpur_dir		<- "data/"
...
filename_utilization	<- paste("../../../data/", trace_type, "/cpu_util-prod-server-", trace_number, "-", trace_type, ".dat", sep = "")
filename_allocation		<- paste("../../../data/", trace_type, "/cpu_alloc-prod-server-", trace_number, "-", trace_type, ".dat", sep = "")

run_analysis(filename_utilization, filename_allocation, trace_number, trace_type, pred_type, num_periods, policies, input_dir, outpur_dir, instance_size_cpu, time_decision, instance_grain, instance_base, pred_grain, cap_grain, error_adjustment, adjustment_type, adjustment_window, slo_limit, capacity_target)
