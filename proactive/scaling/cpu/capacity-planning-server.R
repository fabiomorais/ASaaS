generate_capacity_planning = function(error_adjustment, 
	...
	if (0 %in% ad_w_tmp) {
		...
		capacity_real   	<- complete_trace$CPU_UTIL
		capacity_est		<- estimated_trace$CPU_UTIL
		capacity_est_tg	<- NA
	...
	if (length(ad_w_tmp) != 0) {
		...
		df_correction <- foreach(i = init_it:num_it, .combine = "rbind") %dopar% {
			...
			capacity_real   	<- tmp_df_trace_real$CPU_UTIL
			capacity_est		<- tmp_df_trace_est$CPU_UTIL
			total_cap			<- tmp_df_alloc$CPU_ALLOC
...
run_analysis = function(filename_utilization, filename_allocation, trace_number, trace_type, pred_type, num_periods, policies, input_dir, outpur_dir, util_target, pred_grain, cap_grain, error_adjustment, adjustment_type, adjustment_window, slo_limit) {
	...
	dff_alloc	<- if (file.exists(filename_allocation)) {
					...
					data.frame(TIMESTAMP = dff$TIMESTAMP, CPU_ALLOC = rep(total_cap, nrow(dff)))
				} 
	...
	for (p in 1:length(policies)) { 
		...
		original_df_trace		<- data.frame(df_trace$TIMESTAMP, df_trace$CPU_UTIL * original_df_alloc$CPU_ALLOC / 100, df_trace$EXEC_TIME, df_trace$POLICY, num_period)
		names(original_df_trace)	<- c("TIMESTAMP", "CPU_UTIL", "EXEC_TIME", "POLICY", "PERIOD")
...
policies				<- as.character(args[7])
num_periods			<- as.numeric(args[8])

adjustment_window		<- c(41, 1009, 2017, 4031)
