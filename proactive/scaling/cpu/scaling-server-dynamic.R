...
calculate_violation_timestamp = function(df_trace, scaling_trace, alloc_trace, target_value, target_residual, residual_value) {
	...
	for (i in scaling_trace$TIMESTAMP) {
		...
		total_cap		<- subset(alloc_trace, TIMESTAMP == i)$CPU_ALLOC
		util_value	<- round(subset(df_trace, TIMESTAMP == i)$CPU_UTIL, digits = 1)
...
run_analysis = function(sla_violation_freq, 
	...
	og_df_trace			<- data.frame(df_trace$TIMESTAMP, df_trace$CPU_UTIL * df_alloc$CPU_ALLOC / 100)
	names(og_df_trace)	<- c("TIMESTAMP", "CPU_UTIL")
		...
		while (w <= period_number) {
			...
			for (i in 1:length(ad_w_values)) {
				...
				for (p in 1:length(policies)) { 
					...
					new_df_trace		<- data.frame(tmp_df_trace$TIMESTAMP, tmp_df_trace$CPU_UTIL * tmp_df_alloc$CPU_ALLOC / 100)
					names(new_df_trace)	<- c("TIMESTAMP", "CPU_UTIL")
...
filename_utilization	<- paste("../../../data/normal/cpu_util-prod-server-", trace_number, "-normal.dat", sep = "")
filename_allocation		<- paste("../../../data/normal/cpu_alloc-prod-server-", trace_number, "-normal.dat", sep = "")
