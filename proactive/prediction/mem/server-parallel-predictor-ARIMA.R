...
timez			<- "GMT"
policy_name		<- "arima"
policy_label	     <- "Auto-regressive integrated moving average"
policy_abr		<- "ARIMA"
...
if (!file.exists(file_name)) {
	...
	x <- foreach(i = init_it:num_it) %dopar% {
		...
		try(system(paste("echo ", i/num_it * 100, " > output/result-", trace_number, "-", policy_abr, ".out", sep = ""), intern = TRUE))
		...
		ts_util_core		<- ts(tmp_df_trace$MEM_UTIL)
		...
		if (var(ts_util_core) != 0) {
			...
			arima_util_core		<- auto.arima(ts_util_core, stationary = TRUE, stepwise = FALSE, ic = "aic", max.p = 2016, trace = FALSE, xreg = seq(1,length(ts_util_core)))
			util_core_prediction	<- predict(arima_util_core, n.ahead = pred_ahead, newxreg = seq(1,length(ts_util_core)))$pred[pred_ahead]
	
		}else{
			print("zero variance")
			util_core_prediction <- tmp_df_trace$MEM_UTIL[length(tmp_df_trace$MEM_UTIL)]
		}
