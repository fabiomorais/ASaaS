...
list_of_df <- lapply(list_of_df, function(x) { x$MEM_UTIL <- pmax(x$MEM_UTIL, 0); x})
...
if (!file.exists(file_name)) {
	...
	df_alloc		<- if (file.exists(filename_alloc)) {
					...
					data.frame(MEM_ALLOC = rep(16, nrow(df_util)))
				   }
	...
	cost <-  as.matrix(sapply(list_of_df, function(x) calc_cost(x[1,2], subset(df_trace, TIMESTAMP == start_timestamp)$MEM_UTIL), simplify = TRUE))
	...
	for (t in 2:nrow(list_of_df[["LW"]])) {
		...
		names(tmp_result)	<- c("TIMESTAMP","MEM_UTIL", "EXEC_TIME")
		prediction_result <- rbind(prediction_result, tmp_result)
		
		cost <-  as.matrix(sapply(list_of_df, function(x) calc_cost(x[t,2], subset(df_trace, TIMESTAMP == timestamp)$MEM_UTIL), simplify = TRUE))
...
diff_secs	<- difftime(Sys.time(), begin, units = 'secs')
diff_mins	<- difftime(Sys.time(), begin, units = 'mins')

print(paste(policy_abr, "prediction"))
print(paste("Seconds:", diff_secs))
