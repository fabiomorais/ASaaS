...
df_result      = foreach(tindex = 1:length(jobs), .combine = rbind) %dopar% {      
     ...
     for (flv in 1:nrow(df_flavor)) {
          ...
          for (mt in metrics) {
               ...
               df_res         = df_res %>% mutate(tviol = df_viol_all$viol, p_ecu_viol = ecu_viol / len, p_mem_viol = mem_viol / len, p_viol = tviol / len, jobId = id, flavor = flavor$NAME, metric_base = mt) 
     ...
     filename       = paste("../../../proactive/scaling/2m/data/scaling/single/", id, "-scaling-perd-max-single-index.dat", sep = "") 
     write.table(df_scaling, file = filename, row.names = F)
     
     df_result_tmp
}

write.table(df_result, file = "../data/hp/scaling-analysis-SM-SF.dat", row.names = F)
