...
df_result      = foreach(tindex = 1:nrow(jobs), .combine = rbind) %dopar% {      
     ...
     for (r in 1:nrow(ref)) {
          ...
          for (flv in 1:nrow(df_flavor)) {
               ...
               df_res         = df_res %>% mutate(tviol = df_viol_all$viol, p_ecu_viol = ecu_viol / len, p_mem_viol = mem_viol / len, p_viol = tviol / len, jobId = id, scenario = sc, flavor = flavor$NAME, metric_base = "multi") 
     ...
     filename       = paste("../../../multiple_types/google/data/job/", tmp, "/scaling/single/", id, "-scaling-perd-max-single-index-2m.dat", sep = "") 
     write.table(df_scaling, file = filename, row.names = F)
     
     df_result_tmp
}

write.table(df_result, file = "../data/google/scaling-analysis-MM-SF.dat", row.names = F)
