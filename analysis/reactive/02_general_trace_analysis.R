...
for (metric in metrics) {
     ...
     for (trace in traces) {
          ...
          dff_valc  = diff(ceiling(df_usage[, metric_name]))
          df_result = data.frame(TRACE = trace, 
                                 METRIC = metric, 
                                 SD = sd(dff_val), 
                                 SDC  = sd(dff_valc), 
                                 VAR = var(dff_val), 
                                 VARC = var(dff_valc),
                                 MEAN_USAGE = mean(df_usage[ , metric_name]), 
                                 MEDIAN_USAGE = median(df_usage[, metric_name]), 
                                 MIN_USAGE = min(df_usage[ , metric_name]), 
                                 MAX_USAGE = max(df_usage[, metric_name]))
