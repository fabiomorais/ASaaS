...
tmp            = '27042016T212529'
filename       = paste("output/", tmp, "/conf/", cluster, "_sub_jobs.txt", sep = "")
...
x = foreach(tt = 1:nrow(jobs), .combine = rbind) %dopar% {
  ...
  filename       = paste("output/", tmp, "/pred/normal/", id, "-estimated-trace-AR-168-normal-acf-", wadj, ".dat", sep = "")
  ...
  filename       = paste("output/", tmp, "/scaling/normal/", id, "-scaling-pred-max-index-", target, "-acf-", wadj, ".dat", sep = "")
