...
for (tt in 1:length(jobs)) {
     ...
     filename       = paste("data/pred/normal/", id, "-estimated-trace-AR-168-normal.dat", sep = "")
     df_scaling     = read.table(file = filename, header = T)
     df_scaling     = df_scaling %>% mutate(ECU_UTIL = (CPU_UTIL * trace_ghz) / ecu_factor)
     
     print(head(df_scaling))
     print(tail(df_scaling))
     ...
     filename       = paste("data/scaling/normal/", id, "-scaling-pred-max-index-", target, ".dat", sep = "")
