...
# df_top_freq = df_freq %>% group_by(TRACE, ACT_DONE, PROV_TIME) %>% filter(FREQ == max(FREQ)) %>% sample_n(1) %>% ungroup() %>% select(TRACE, ACT_DONE, THRESHOLD, THR_FREQ = FREQ)
...
write.table(df_freq_selection, file = "data/pre_reactive_scenarios_mem.dat", row.names = F)
...
df_threshold                  = df_threshold %>% filter(ADD < 1, RM < 0.9)
...
write.table(df_out, file = "data/reactive_scenarios_mem.dat", row.names = F, col.names = F)
